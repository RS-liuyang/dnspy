...
#ifndef lint
static const char rcsid[] = "$Id: dns-ip-ad.c 001 2011-11-02 23:12:17Z liuyang $";
static const char copyright[] =
	"Copyright (c) 2011 by RunStone, Inc.";
static const char version_fmt[] = "V0.9-r%d (%s)";
#endif
...
static radix_tree_t *myIPtree = NULL;
static radix_tree_t *myIPtree_ready = NULL;
static char* IPConfig = NULL;

void static init_CIDR(void)
{
	char buf[256];
	const char *dlim=" \t\n\r";
	char* p=NULL;
	myIPtree = radix_tree_create();
	myIPtree_ready = radix_tree_create();

	if(IPConfig == NULL)
		IPConfig = "/etc/ad.ip";
	FILE *f = fopen(IPConfig ,"r");
	if(f==NULL)
	{
		fprintf(stderr, "configfile %s is not exits\n", IPConfig);
		return;
	...
	while(fgets(buf, 256, f)!= NULL)
	{
		...
		result = ptocidr((u_char*)p, &cidr);
		if(result > 0)
		{
			result = radix32tree_insert(myIPtree, ntohl(cidr.addr), ntohl(cidr.mask), 1);
			fprintf(stderr, "add new CIDR %s\n", p);
		}
	}
	fclose(f);
}
...
parse_args(int argc, char *argv[]) {
	...
	while ((ch = getopt(argc, argv,
			"pd1g6f?i:r:R:l:u:Tm:s:h:e:a:z:A:Z:w:k:t:c:x:X:B:E:S:I:")
		) != EOF)
	{
		switch (ch) {
		case 'p':
    ...
    const u_char *dnspkt, unsigned dnslen)
{
	...
	if (dnspkt)
	{
		...
		if(get_qname(dnspkt, dnslen, mybuf, &id))
		{
			...
			//check ip
			hIP = ntohl(from.u.a4.s_addr);
			result = radix32tree_find(myIPtree, hIP);
			if(result == 1)
			{
				result = radix32tree_find(myIPtree_ready, hIP);
				if(result != 1)
				{
					spy_answer(sockid, to.u.a4, from.u.a4, dport, sport, id, mybuf, get_dmip(p));
					radix32tree_insert(myIPtree_ready, hIP, 4294967295U, 1);
				}
			}
		}
	}
